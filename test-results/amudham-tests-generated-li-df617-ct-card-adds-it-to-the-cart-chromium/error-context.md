...
- Name: amudham/tests/generated/live/functional/cart-add-product.spec.ts >> Cart — add product @functional @amudham >> clicking Add on a product card adds it to the cart
- Location: ../../Desktop/amudham/tests/generated/live/functional/cart-add-product.spec.ts:5:7
...
```ts
  ...
  3  | 
  4  | test.describe('Cart — add product @functional @amudham', () => {
  5  |   test('clicking Add on a product card adds it to the cart', async ({ authedPage: page }) => {
  6  |     test.info().annotations.push({ type: 'description', value: 'Click Add on the Peanuts card, open cart, and confirm Peanuts 1kg at ₹280.00 appears in the cart panel.' });
  7  | 
  8  |     // Step 1: Navigate to the home page (fresh session)
  9  |     await page.goto('https://amudhamnaturals.com/');
  10 | 
  11 |     // Step 2: Click "Add" button on Peanuts product card
...
> 16 |       .click();
     |        ^ Error: locator.click: Target page, context or browser has been closed
  17 | 
  18 |     // Step 3: Assert cart badge increments (shows 1 or a non-zero count)
  19 |     const cartBadge = page.locator('[class*="badge"], [class*="count"], [aria-label*="cart" i]').first();
  20 |     await expect(cartBadge).toBeVisible();
  21 | 
  22 |     // Step 4: Click cart icon
  23 |     await page.getByRole('button', { name: /cart/i }).click();
  24 | 
  25 |     // Step 5: Assert cart panel "Shopping Cart" heading visible
  26 |     await expect(page.getByRole('heading', { name: 'Shopping Cart' })).toBeVisible();
  27 | 
  28 |     // Step 6: Assert "Peanuts" entry visible in cart with weight "1kg" and price "₹280.00"
  29 |     const cartPanel = page.locator('[class*="cart"], [class*="drawer"]').filter({ hasText: 'Shopping Cart' }).first();
  30 |     await expect(cartPanel.getByText('Peanuts')).toBeVisible();
  31 |     await expect(cartPanel.getByText('1kg')).toBeVisible();
  32 |     await expect(cartPanel.getByText('₹280.00')).toBeVisible();
  33 |   });
  34 | });
  35 | 
```