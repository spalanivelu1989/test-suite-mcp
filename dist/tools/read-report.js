...
export function readReport(app, runDir) {
    let targetDir;
    if (runDir) {
        ...
        if (!existsSync(runs)) {
            throw new Error(`No runs found for app "${app}". Run /test-app ${app} first.`);
        }
    ...
    if (!existsSync(reportPath)) {
        throw new Error(`report.md not found in ${targetDir}. Run /test-app first.`);
    }
