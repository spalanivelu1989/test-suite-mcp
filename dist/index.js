...
import { initPlaywrightProxy, getBrowserTools, isBrowserTool, callBrowserTool, } from "./lib/playwright-proxy.js";
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "read_app_model",
            description: "Read the most recent app-model.json for an app (written by /discover). " +
        ...
        {
            name: "list_runs",
            description: "List run directory names for an app, newest first. " +
        ...
        {
            name: "read_report",
            description: "Read the report.md from the most recent (or a specific) test run. " +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    run_dir: {
                        type: "string",
                        description: "Specific run directory name (e.g. 2026-05-14T09-30-12). " +
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        ...
        switch (name) {
            ...
            case "read_app_model": {
                const model = readAppModel(args.app);
                return {
                    content: [{ type: "text", text: JSON.stringify(model, null, 2) }],
                };
            }
            case "list_runs": {
                const runs = listRuns(args.app);
                return {
                    content: [{ type: "text", text: JSON.stringify(runs, null, 2) }],
                };
            }
            case "run_playwright": {
                const results = runPlaywright(args.app, args.category);
                return {
                    content: [{ type: "text", text: JSON.stringify(results, null, 2) }],
                };
            }
            case "read_report": {
                const report = readReport(args.app, args.run_dir);
                return {
                    content: [
...
const PROMPT_DEFS = [
    {
        name: "add-app",
        description: "Interview the user to scaffold a new app under test. " +
    ...
    {
        name: "discover",
        description: "Orchestrate a crawl of the deployed app: detect auth flow, log in per role, " +
    ...
    {
        name: "design",
        description: "Generate Playwright .spec.ts files across 5 categories from app-model.json " +
            "and description.md.",
        arguments: [
            {
                name: "app_name",
    ...
    {
        name: "test-app",
        description: "Run the full Playwright suite, diagnose failures, and produce a report " +
