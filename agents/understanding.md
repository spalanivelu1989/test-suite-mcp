...
5. Save the storage state to **`apps/<app>/.auth/<role>.json`** (stable path; the timestamped `runs/` dir is for artifacts, not for spec dependencies). If Playwright MCP does not expose a direct `context.storageState()` export, fall back to running a small Playwright CLI dump via `Bash` — and **note in your output that this side-channel was used** so the orchestrator knows.
...
```json
{
  ...
  "roles": [
    {
      "id": "admin",
      "storage_state": "apps/<app>/.auth/admin.json",
      "landing_url": "...",
      "login_ok": true,
      "blocker": null
    }
